<template>
    <div>
        <button @click="sendUpdateNotification">模擬送貼文</button>
        <button @click="updateUserFollowers">模擬更新追蹤名單</button>
        <P>{{ message }}</P>
    </div>
</template>
...
const updateUserFollowers = () => {
    socket.emit('updateUserFollowers', ['628fc03af680ed29550907e9']);
};
